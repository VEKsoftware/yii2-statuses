...
class StatusesLinks extends CommonRecord
{
    ...
    {
        return [
            [['status_from', 'status_to'], 'required'],
            [['status_from', 'status_to'], 'integer'],
//            [['right_tag'], 'string'],
//            ['right_tag', 'match', 'pattern'=>'/^[a-zA-Z0-9-_\.]+$/'],
            [['status_from', 'status_to'], 'validateExists'],
        ];
    }
    ...
    {
        if ($insert) {
           $this->right_tag = "{$this->statusFrom->docType->symbolic_id}.{$this->statusFrom->symbolic_id}.{$this->statusTo->symbolic_id}";
        }
        return true;
    }
